...
def to_cumulative(stream: list):
    ...
    for i in range(len(stream)):
        ...
        if data[0] in result:
            if data[1] in result[data[0]]:
                ...
                result[data[0]][data[1]][1] = result[data[0]][data[1]][1] + int(data[2])*oneDpToInt(data[3])
            else:
                result[data[0]][data[1]] = [int(data[2]),int(data[2])*oneDpToInt(data[3])]
        else:
            result[data[0]] = {}
            result[data[0]][data[1]] = [int(data[2]),int(data[2])*oneDpToInt(data[3])]
    ...
    for i in sorted(result.keys()):
        ...
        for j in sorted(result[i].keys()):
            ...
            temp +=  "," + str(j) + "," + str(sum_vol[j]) + "," + intToOneDp(sum_turnover[j])
